...
(function () {
    ...
    var d = document, isStrict = d.compatMode == "CSS1Compat", dd = d.documentElement, db = d.body, m = Math.max,
        ie = !!d.all, ua = navigator.userAgent.toLowerCase(), head = d.getElementsByTagName("head")[0] || dd,
        wlh = window.location.host, conf = (typeof(jiathis_config) == 'undefined') ? {} : jiathis_config,
        _ckpre = JIATHIS_CONFIGS.ckprefix, _lists = JIATHIS_CONFIGS.servicelist, _ref = d.referrer, _reced = false,
        getWH = function () {
            ...
            e.showType && creElm({src: JIATHIS_CONFIGS.codehost + "/plugin.shareimg.js", charset: "utf-8"}, "script", head);
            var f = "qzone,tsina,tqq,weixin,renren,kaixin001,evernote,linkedin,douban,ydnote,xiaoyou,msn,fb,twitter,tieba,baidu,google",
                _jck = JIATHIS_CONFIGS.jtck || f, jck = _uniqueConcat(_jck.split(","), f.split(",")), parentServices = {},
                _WR = {}, h = d.getElementsByTagName("a"), _url = String(conf.url || d.location), _CF = null, webid, likeid,
                tl, fl, bt, preferred;
            for (var i = 0, ci, tmp; ci = h[i++];) {
                ...
                if (ci.className && (tmp = ci.className.match(/^jiathis_counter_(\w+)(?:\_|$)(.*)$/)) && tmp[1]) {
                    if (typeof($CKE.containers) == "object") {
                        ...
                        if (!ci.firstChild) {
                            var B = tmp[1] == 'style' ? 'bubble' : tmp[1], C = tmp[2] ? tmp[2] : '',
                                K = "jiathis_counter_" + i, E = d.createElement("span");
                ...
                if (ci.className && (tmp = ci.className.match(/^jiathis_like_(\w+)$/)) && tmp[1]) {
                    ...
                    if (likeid == 'qzone') {
                        ...
                        var p = jiathis_get_des(), pic = '', title = conf.title || document.title,
            ...
            var a, s, E = encodeURIComponent, o = _grf(_ref), T = document.title || "", Y = window.location.href || "",
                an = Y ? Y.indexOf(JIATHIS_CONFIGS.jtcbk) : -1, d1 = _gd(o.host), d2 = _gd(Y), q = null,
            ...
            var a = jiathis_get_des(), pic = jiathis_get_pic(),
    ...
    creElm({href: JIATHIS_CONFIGS.codehost + "css/jiathis_share.css", rel: "stylesheet", type: "text/css"}, "link");
    $CKE = {
        jid: "", pop: div, centerpop: div1, shares: 0, containers: [], disappear: function (a) {
            var b = window.event || a, t = b.srcElement || b.target, tn = t.tagName ? t.tagName.toUpperCase() : "",
                c = div.contains ? div.contains(t) : !!(div.compareDocumentPosition(t) & 16),
                c1 = div1.contains ? div1.contains(t) : !!(div1.compareDocumentPosition(t) & 16), c2 = true;
            if (tn == "IMG") {
            ...
            if (o.shares !== undefined) {
                var A = $CKE.containers, B = parseInt(o.shares), C = String(conf.url || d.location), D = _gck(),
                    J = _rck(D[C]), T = (new Date()).getTime(), S = B;
                if (J && J.shares > B) {
                    S = J.shares
                }
                _sck(C, S, T);
    ...
    if (!conf.do_not_track && wlh && typeof(_gnayTrack) != 'object') {
        d.write('<script type="text/javascript" src="' + JIATHIS_CONFIGS.codehost + 'js/plugin.client.js" charset="utf-8"></script>')
    }
})();
...
function jiathis_sendto(a) {
    ...
    ;var d = encodeURIComponent, cu = JIATHIS_CONFIGS.custom[a] || {}, U = String(c.url || document.location),
        W = "?webid=" + a, G = "&url=" + d(U), T = "&title=" + d(c.title || document.title),
        S = c.summary ? "&summary=" + d(c.summary) : (b ? "&summary=" + d(b) : ""),
        F = JIATHIS_CONFIGS.uid ? "&uid=" + parseInt(JIATHIS_CONFIGS.uid) : "",
        E = c.data_track_clickback ? "&jtss=1" : "", K = (c.appkey && c.appkey[a]) ? "&appkey=" + c.appkey[a] : "",
        P = c.pic ? "&pic=" + d(c.pic) : (pic ? "&pic=" + d(pic) : ''), C = $CKE.jid ? "&jid=" + $CKE.jid : "",
        R = (c.ralateuid && c.ralateuid[a]) ? "&ralateuid=" + c.ralateuid[a] : "",
        Q = (c.evt && c.evt['share']) ? c.evt['share'] : null, A = 'http://s.jiathis.com/',
...
function jiathis_get_pic() {
    ...
    for (i = 0; i < a.length; i++) {
        var b = parseInt(a.item(i).offsetWidth), imgH = parseInt(a.item(i).offsetHeight), minW = 300, minH = 150,
            width = (300 / imgH) * 150, height = (150 / b) * 300;
        if (b >= minW && imgH >= minH) {
            if ((width - height) <= 150) {
